defmodule OnagalWeb.Middleware.SafeResolution do
  alias Absinthe.Resolution
  require Logger

  @behaviour Absinthe.Middleware
  @default_error {:error, :unknown}
  @invalid_record {:error, :not_found}
  @invalid_update {:error, :validation}
  ...
  def call(resolution, resolver) do
    ...
    exception in Ecto.NoResultsError ->
      ...
      Logger.error(error)
      Resolution.put_result(resolution, @invalid_record)

    exception in Ecto.Changeset ->
      error = Exception.format(:error, exception, __STACKTRACE__)
      Logger.error(error)
      Resolution.put_result(resolution, @invalid_update)
